...
class ReceptiveField():
    ...
    def __init__(self, graph, w, s=1, k=10, l='betweenness', default=None, attribute_name='node_attributes', rank_name='node_rank'):
        self.graph = graph
        self.w = w
        self.s = s
        self.k = k
    ...
    def normalize_graph(self, neighbourhood, v, node_labels):
        # Compute ranking:
        # d(u, v) < d(w, v) -> r(u) < r(v)
        ranking = self.compute_ranking(neighbourhood, v, node_labels)
        # |U| > k
        if len(ranking) > self.k:
            ...
            subgraph_ranking = self.compute_ranking(subgraph, v, subgraph_labels)
        # |U| < k
    ...
    def compute_ranking(self, graph, v, node_labels, canonical=None):
        ...
        for d in distinct_distances:
            ...
            if canonical:
                current_nodes = sorted(current_nodes, key=lambda x: (-node_labels[x], canonical[x]))
            else:
